...
import { onlinePeers$ } from "@/lib/features/sync/observables";
...
export async function setupWebrtcReplication(
  ...
  const isActive = useSyncStore.getState().isActive;
  if (!isActive) {
    console.log("Sync is not active. Skipping WebRTC replication setup.");
    return;
  }
  ...
  const enabledReplications: string[] = [];
  ...
  for (const collection of availableCollections) {
    ...
    if (collection.webrtcEnabled) {
      try {
        ...
        enabledReplications.push(collectionName);
      } catch (error) {
        continue;
      }
    }
  ...
  if (enabledReplications.length > 0) {
    console.log(
